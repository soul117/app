...
public class Ball extends AGameObject {
    ...
    public Ball(Context context) {
        this(context, UserSettings.getSpeed());
    }

    public Ball(Context context, float speed) {
        this(context, speed, 75);
    }

    public Ball(Context context, float speed, int radius) {
        this(context, speed, radius, Color.WHITE);
    }

    public Ball(Context context, float speed, int radius, int color) {
        this(context, speed, radius, color, R.drawable.ball);
    }
    //constructor
    public Ball(GameView gameView, float speed, int radius, int color, int resource_id) {
        this(gameView.getContext(), speed, radius, color, resource_id);
        this.setGameView(gameView);
    }
    //finally constructors initialized
    public Ball(Context context, float speed, int radius, int color, int resource_id) {
        ...
        this.setImage(BitmapFactory.decodeResource(context.getResources(), resource_id));
    ...
    public void update() {
        ...
        if (PlayerInfo.isClicked && getGameView().isClickOnBascket()) {
        ...
        if ((b != null && deltaX < 100 && deltaY < 100) ||
            ...
            this.getGameView().createNewBallRandomly(getContext(), this);
        }
    }
